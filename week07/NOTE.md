...
当 css 规则应用后，浏览器会开始计算其在屏幕上显示的位置和占据的空间大小.

### *toy-browser*中的layout使用弹性布局(display: inline-flex)。

最主要实现以下几点：

1. 计算容器的高度和宽度
2. 项目除了高度和宽度，还有项目在容器里的位置
3. 主轴、交叉轴方向的布局
4. flex-direction



`flex容器`的`主轴`的长度是由**第一行**的`项目`的主轴方向的长度之和。

`flex`容器的`交叉轴`的长度是由每行的中交叉轴方向的最长的项目之和。
...
function layout(element) {
  ...
  if (elementStyle.display !== 'flex') return; // 只处理有flex
